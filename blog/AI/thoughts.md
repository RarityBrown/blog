...
2025/10/26: opus-4.1 本质上是一个专用模型，它不擅长所有非 benchmark-style 或 coding 的题目；而 gpt-5 其实是一个较为通用的模型，它擅长一切不那么小众的所有话题，除了如何表达；qwen4 绝对值得期待，qwen3-next 展现出了这个尺寸下相当强的能力。

2025/11/23: Gemini 3 的知识量可以用恐怖来形容，可能是历史上开放给用户使用的 LLM 中总参数量最大的，很可能比 Gemini 1.0 Ultra 和 GPT-4.5 都要大。在大部分 benchmark 上确实是越大越强，可惜边界是客观存在的，线性的能力提升，需要对数的训练语料和模型参数增长，而这个对数增长又远快于 Moore 定律可以提供的性能，当然可以用上世纪的 Mainframe 观点来提升性能，这也正是全球在做的事情，但是最终的一切还是要靠 Moore 定律的，因为靠 Mainframe 至多只能凑出一个数量级的提升。所以与年初的乐观不同（即预测 2025 年底 HLE 和 SciCode Main 40%+ ，其中 HLE 因为名字比较讨喜，大家都很关注，可以算基本预测正确；SciCode Main 则是因为没有人关注，还在 10%+ 徘徊）。如果是 GPQA 和 MMLU-Pro 从主流的 85% 到 Gemini 3 可以提供的 90%，需要接近 10 倍的预训练提升，那么很难想象 2026 年从 90% 至 95% 来正确解决这些长尾知识问题所需要付出的努力。所以悲观的认为，2026 年年底一流的 HLE wo web wo python 成绩会在 50% 左右，而不同于 Epoch AI 拟合出的那些类逻辑斯蒂曲线（即增长最快的阶段）。这是因为 HLE 中 40% 是数学题，可以通过不增加参数量的 Post-train 来提高分数，但是一旦接近 40% 这个屏障，唯有加大参数量可以提升分数。

2025/11/23: 2026 的 post-train 和 2025 一样会展现出勃勃生机，只不过还是手里榔头眼中钉——需要在什么 benchmark 上表现好一点，就 post-train 一点类似的数据集，在通用性上毫无帮助，但是在商业的角度应该可以实现正营收。Agent 特别是 SWE-Agent 应该会迎来更大发展，很难想象在 2025 年底，Codex CLI, gemini-cli; Cursor, Antigravity 直接读取 pdf 文件的支持都还接近于没有。

2025/11/23: 如果 2023 AI 代表词是 GPT-4, 2024 是 o1, 2025 则是 nano banana pro 了，开源模型有希望在 2026 年底达到相近的效果，这会比 r1 之于 o1 的一个季度慢上不少。
...
2026/02/07: gpt-5.3 和 opus-4.6 基本上还是在打 coding 的仗，与我无关。但是 [gpt-5-lowers-protein-synthesis-cost](https://openai.com/index/gpt-5-lowers-protein-synthesis-cost/) 则是不容小觑。证明了在高度规范化的条件下，AI 对物理世界而非代码世界 "优化" 的可行性。

2026/02/08: glm-ocr + ollama 伟大！本地万岁！

2026/02/25: 《和物理世界打交道》：如果按照营收计算，OpenAI 在 2026 年大约可以排在全球 500 强中的前 20 名。一个全球排名前 20 的代码公司，几乎用尽了所有可以得到的代码数据，得到了一个代码水平和从业 5\~10 年左右程序员相似代码水平的模型。而 GE 在 2026 年大约在全球 500 强中排名 100，和 OpenAI 相比，人工智能工程师更少，可供训练的原始数据更少，几乎不可能得到一个机械设计水平和从业 5\~10 年左右工程师相似水平的模型。所以如果 OpenAI 不再追求或者难以继续追求通用能力上升时，只有代码行业（暂时忽略翻译等小众行业）会被替代，而远远不是所谓的“所有白领”。开发一个领域专业模型，需要投入人力物力的量级远不止 Alpha Go，所以我目前粗浅的认为，所有宣传 AI 已经深度融入传统工作流并可以获得（而非辅助获得）几乎可用的最终交付结果的公司，全是扯淡，并且在未来 10 年内也不会出现。         宇树机器人可以拍炫酷视频，但是就是没法帮我买菜，欢迎 AI 来到物理世界！

2026/03/08: Remote Labor Index (RLI) 是一个值得关注的指标，Junior-SWE 会在 2026 年底被替代，但是 RLI 中 Junior 的活还是有点难。特别是如果还是用 screenshot -> action 这个 loop 来做。

2026/08/28：《和仿真世界打交道》：Jalapeño 可以通过近 1/50 的工程师体量，在某些 spec 上基本打平 Nvidia。如果 Nvidia 可以招到全世界最优秀的 50 个架构和数字 IC 工程师，和 4950 个次优秀的工程师；而 OpenAI 的 LLM 可以达到第 51 名的水平。谁会开发出更优秀的芯片？当然，我们的前提是 OpenAI 不给 Nvidia 使用最先进的模型；Nvidia 由于数据安全，也不敢使用 OpenAI 的模型进行真实工程开发。所以如果是普通人，应该选择三个臭皮匠顶个诸葛亮的行业，还是三个臭皮匠顶不了诸葛亮的行业？
