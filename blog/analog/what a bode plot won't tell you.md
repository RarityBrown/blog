...
### 零点不会影响 GBW，但是确实会影响建立速度

因为 GBW 和 τ 的近似前提是单极点系统

```markdown
$$ 
H_1(s) = A_0 \cdot \frac{(1 + \frac{s}{\omega_{z1}}) }{(1 + \frac{s}{\omega_{p1}}) (1 + \frac{s}{\omega_{p2}}) (1 + \frac{s}{\omega_{p3}})} \\
H_2(s) = A_0 \cdot \frac{1}{1+\frac{s}{\omega_{p1}}}
$$

* **直流增益 $A_0$** = $67 \text{ dB}$
* **主极点 $f_{p1}$** = $38 \text{ kHz}$
* **左半平面零点 $f_{z1}$** = $50 \text{ MHz}$
* **次极点 $f_{p2}$** = $120 \text{ MHz}$
* **高频寄生极点 $f_{p3}$** = $250 \text{ MHz}$

图一：这两个系统都接成单位增益负反馈后，在同一幅图上画出两者的 step response，以及再同一幅图上画出相对建立误差和时间的关系（对数轴和线性轴都要）。所以这幅图总共有 3 个 y 轴和 6 条线。

图二：$H_1(s)$ 可以分解为三个 exp 的时域响应，把这三个也画在同一幅图上

图三：画出两者开环下的的幅频和相频响应，这个也在一张图上，标注出 GBW, UGB, phase margin
```
